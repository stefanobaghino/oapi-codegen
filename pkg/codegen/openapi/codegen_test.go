...
const (
	remoteRefFile = `https://raw.githubusercontent.com/stefanobaghino/oapi-codegen/master/examples/petstore-expanded` +
		`/petstore-expanded.yaml`
	remoteRefImport = `github.com/stefanobaghino/oapi-codegen/v2/examples/petstore-expanded`
)
...
func TestExampleOpenAPICodeGeneration(t *testing.T) {
	...
	swagger, err := LoadFromData([]byte(testOpenAPIDefinition))
	assert.NoError(t, err)
...
func TestRemoteExternalReference(t *testing.T) {
	...
	// Check import
	assert.Contains(t, code, `externalRef0 "github.com/stefanobaghino/oapi-codegen/v2/examples/petstore-expanded"`)
